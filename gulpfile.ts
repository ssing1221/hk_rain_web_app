...
gulp.task("compile", function (callback) {
    runSequence('tslint_server', 'tslint_client');
});
...
gulp.task('watch', function () {
    gulp.watch(["client/**/*.ts"], ['compile']).on('change', function (e) {
        console.log('client typeScript file ' + e.path + ' has been changed. Compiling and building...');
        runSequence('build:client', 'clientResources');
    });

    gulp.watch(["client/**/*.html", "client/**/*.css"], ['clientResources']).on('change', function (e) {
        console.log('Resource file ' + e.path + ' has been changed. Updating.');
        runSequence('clientResources');
    });

    gulp.watch(["server/src/**/*.ts"], ['compile']).on('change', function (e) {
        console.log('Server typeScript file ' + e.path + ' has been changed. Need server restart!!!');
